...
impl Aes128Ref {
    ...
    pub fn get_keyed_inv_sbox_mult_by_E(&self, round: usize) -> [[u8; 256]; 16] {
        let mut keyed_sbox = [[0u8; 256]; 16];
        let round_key = self.get_round_keys()[round];
        ...
        for (byte_idx, rk) in round_key.iter().enumerate() {
            for x in 0..256 {
                ...
                keyed_sbox[byte_idx][x as usize] = AES128_INV_SBOX_MULT_BY_E[masked_input as usize];
            }
        }
        keyed_sbox
    }

    pub fn get_keyed_inv_sbox_mult_by_B(&self, round: usize) -> [[u8; 256]; 16] {
        let mut keyed_sbox = [[0u8; 256]; 16];
        let round_key = self.get_round_keys()[round];
        ...
        for (byte_idx, rk) in round_key.iter().enumerate() {
            for x in 0..256 {
                ...
                keyed_sbox[byte_idx][x as usize] = AES128_INV_SBOX_MULT_BY_B[masked_input as usize];
            }
        }
        keyed_sbox
    }

    pub fn get_keyed_inv_sbox_mult_by_D(&self, round: usize) -> [[u8; 256]; 16] {
        let mut keyed_sbox = [[0u8; 256]; 16];
        let round_key = self.get_round_keys()[round];
        ...
        for (byte_idx, rk) in round_key.iter().enumerate() {
            for x in 0..256 {
                ...
                keyed_sbox[byte_idx][x as usize] = AES128_INV_SBOX_MULT_BY_D[masked_input as usize];
            }
        }
        keyed_sbox
    }

    pub fn get_keyed_inv_sbox_mult_by_9(&self, round: usize) -> [[u8; 256]; 16] {
        let mut keyed_sbox = [[0u8; 256]; 16];
        let round_key = self.get_round_keys()[round];
        ...
        for (byte_idx, rk) in round_key.iter().enumerate() {
            for x in 0..256 {
                ...
                keyed_sbox[byte_idx][x as usize] = AES128_INV_SBOX_MULT_BY_9[masked_input as usize];
            }
        }
        keyed_sbox
    }
