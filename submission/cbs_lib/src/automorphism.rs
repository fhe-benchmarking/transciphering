...
pub struct AutomorphKeySerializable {
    ksk_data: Vec<u64>,
    decomp_base_log: DecompositionBaseLog,
    decomp_level_count: DecompositionLevelCount,
    glwe_dimension: GlweDimension,
...
impl AutomorphKey<ABox<[c64]>> {
    pub fn to_serializable(&self) -> AutomorphKeySerializable {
        // 将 c64 数据转换为 u64 进行序列化
        let mut ksk_data = Vec::new();
        for &c64_val in self.ksk.as_fourier_glev_ciphertext_list().as_ref() {
            ksk_data.push(c64_val.re.to_bits());
            ksk_data.push(c64_val.im.to_bits());
        }
        
        AutomorphKeySerializable {
            ksk_data,
    ...
    pub fn from_serializable(
        serializable: AutomorphKeySerializable,
        fft_type: FftType,
    ) -> Self {
        let glwe_size = serializable.glwe_dimension.to_glwe_size();
        let ksk = FourierGlweKeyswitchKey::new(
            glwe_size,
            glwe_size,
            serializable.polynomial_size,
            serializable.decomp_base_log,
            serializable.decomp_level_count,
            fft_type,
        );
        ...
        for c64_val in ksk_mut.as_mut_fourier_glev_ciphertext_list().as_mut() {
            if idx + 1 < serializable.ksk_data.len() {
                *c64_val = c64 {
                    re: f64::from_bits(serializable.ksk_data[idx]),
                    im: f64::from_bits(serializable.ksk_data[idx + 1]),
                };
                idx += 2;
            }
        }
        ...
        AutomorphKey {
            ksk: ksk_mut,
            decomp_base_log: serializable.decomp_base_log,
            decomp_level_count: serializable.decomp_level_count,
            glwe_dimension: serializable.glwe_dimension,
            polynomial_size: serializable.polynomial_size,
            auto_k: serializable.auto_k,
        }
    }
}